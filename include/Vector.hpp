...
/*   Updated: 2022/09/08 20:03:16 by mamartin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
namespace ft
{
	...
	struct Vector
	{
		public:
			...
			/* Copy from upper dimension */
			Vector(const Vector<L + 1, T>& rhs) : Vector()
			{
	...
	template <length_t L, typename T>
	Vector<L, T> rotate(Vector<L, T> v, T angle, const Vector<3, T> axis)
	{
		Vector<3, T> k = normalize(axis);
		const float cos = std::cos(angle);
		const float sin = std::sin(angle);
	
		v = v * cos + cross_product(k, v) * sin + k * k.dot(v) * (1 - cos);
		return v;
	}
}
